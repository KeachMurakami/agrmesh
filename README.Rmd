...
Rstudioでの利用を想定しています。
このパッケージはApple siliconのMacとWindows 11で動作することが確認されています。
...
2026年度からは**研究・開発・教育・試用ユーザ (以下, 検証ユーザ)**と**商用・業務ユーザ (以下, 一般ユーザ)**で認証方式が異なります。
パッケージ読み込み時は、検証ユーザ向けの認証方式を利用するモードになっています。

### 検証ユーザ

システムの利用に[Oracle Identity Cloud Service (IDCS) を利用した多要素認証](https://amu.rd.naro.go.jp/wiki_open/doku.php?id=faq)が必要です。
...
`XXXX`にはたデータ提供企業から提供されるopendapサーバのURLを指定してください。
