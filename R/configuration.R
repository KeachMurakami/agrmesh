...
amgsds_config <-
  function(userid = Sys.getenv("amgsds_id"), password = Sys.getenv("amgsds_pw"), .force = FALSE){
    ...
    if(is_network_available){
      # verify id/pw
      tryCatch({
        connect <- readr::read_lines(stringr::str_glue("https://{userid}:{password}@amd.rd.naro.go.jp/opendap/AMD/"))
