...
generate_path <-
  function(times, lats, lons, element, source = "daily",
    ...
    if(source %in% c("daily", "scenario")){
      ...
      if(is_hour_explicit){
        start_hour <- lubridate::hour(min(times))
        end_hour <- lubridate::hour(max(times))
      } else {
        start_hour <- 1
        end_hour <- 24
      }
      time_index <- range(lubridate::yday(times)) * 24 - c(24, 1) + c(start_hour - 1, end_hour - 24)
      if(time_index[1] < 0){
...
#' @param .silent suppress downloader messages if `TRUE`
#' @export
download_netcdf <-
  function(amgsds_path, outdir, .silent = TRUE){

    remote <- stringr::str_glue('https://{Sys.getenv("amgsds_id")}:{Sys.getenv("amgsds_pw")}@amd.rd.naro.go.jp/opendap/')
...
fetch_amgsds <-
  function(times = Sys.time(),
    ...
    if(length(elements) == 1){
      ...
      tempdir <- tempdir()
      download_netcdf(amgsds_path[[mode]], tempdir, .silent = .silent)
      ...
      result <-
        purrr::map(elements,  ~ fetch_amgsds(times, lats, lons, ., mode, source, output, model, RCP, is_clim, .silent))
      if(output == "tibble"){
        common_names <-
  ...
  function(times = Sys.time(),
    ...
    if(length(elements) == 1){
      ...
      if(sum(not_exist & is_available) > 0 & autodownload){
        ...
        download_netcdf(amgsds_path = amgsds_path[["complete"]][not_exist & is_available], outdir = localdir, .silent = .silent)
